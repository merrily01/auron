...
# Utility for creating well-formed pull request merges and pushing them to Auron
# For committers:
# Please check your local git envs via `git remote -v` which should
# apache	git@github.com:apache/auron.git (fetch)
# apache	git@github.com:apache/auron.git (push)
# origin	git@github.com:[ YOUR GITHUB USER NAME ]/auron.git (fetch)
# origin	git@github.com:[ YOUR GITHUB USER NAME ]/auron.git (push)
...
AURON_HOME = os.environ.get("AURON_HOME", os.getcwd())
PR_REMOTE_NAME = os.environ.get("PR_REMOTE_NAME", "apache")
PUSH_REMOTE_NAME = os.environ.get("PUSH_REMOTE_NAME", "apache")
GITHUB_OAUTH_KEY = os.environ.get("GITHUB_OAUTH_KEY")
GITHUB_API_BASE = "https://api.github.com/repos/apache/auron"
    ...
    try:
        ...
        if "X-RateLimit-Remaining" in e.headers and e.headers["X-RateLimit-Remaining"] == '0':
            print("Exceeded the GitHub API rate limit; see the instructions in " +
                  "dev/merge_auron_pr.py to configure an OAuth token for making authenticated " +
                  "GitHub requests.")
        else:
...
def fix_title(text, num):
    if (re.search(r'^\[AURON\s#[0-9]{3,6}\].*', text)):
        return text

    return '[AURON #%s] %s' % (num, text)
...
def merge_pr(pr_num, target_ref, title, body, pr_repo_desc):
    ...
    if body is not None:
        ...
        # to people every time someone creates a public fork of Auron.
    ...
    for issueId in re.findall("AURON #[0-9]{3,5}", title):
        merge_message_flags += ["-m", issueId.replace("AURON", "Closes")]
